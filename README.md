...
```bash
git clone https://github.com/Raksj12/Loanscope.git
cd Loanscope
npm install
```
 
> **Note on dependencies:** this is a Node.js project, so dependencies are declared in `package.json` (the Node.js equivalent of a Python `requirements.txt`) and installed with `npm install`, as shown above — there is no `requirements.txt` file because npm is the package manager for this stack, not pip.
